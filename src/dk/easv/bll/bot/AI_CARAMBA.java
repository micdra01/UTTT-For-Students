...
public class AI_CARAMBA implements IBot {
    private static final String BOTNAME = "¡AI CARAMBA!";
    ...
    public IMove doMove(IGameState state) {
        if (calculateMove(state) != null) {
            return calculateMove(state);
        }
        return doPrefMove(state);
    }
     ...
     */
    public IMove doPrefMove(IGameState state) {
    ...
    private IMove calculateMove(IGameState state) {
        ...
        for(IMove move : moves) {
            simulator.updateBoard(move);
            if(simulator.isWin(simulator.getCurrentState().getField().getBoard(), move, String.valueOf(simulator.currentPlayer))) {
                return move;
            }
        }
        return null;
